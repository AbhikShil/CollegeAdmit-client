...
const OrdersCollege = ({ user , ordersData, handleStatusChange }) => {
  ...
  return (
    <>
      {ordersData.map((order) =>{return (order.orderStatus!="Profile Eligible .Seat Is Successfully Booked." && order.orderStatus!="Profile Rejected. Apply Again!")?(
        
        <div key={order._id} className="row pb-5 p-3 card">
